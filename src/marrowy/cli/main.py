...
async def _console_poller(conversation_id: str) -> None:
    rendered_regular_ids: set[str] = set()
    ...
    while True:
        ...
        try:
            ...
            for message in messages:
                ...
                if metadata.get("streamMessage"):
                    state = stream_state.setdefault(
                        message.id,
                        {"content": "", "thinking_count": 0, "opened": False, "closed": False},
                    )
                    thinking = list(metadata.get("thinking", []))
                    for entry in thinking[state["thinking_count"] :]:
                        text = str(entry.get("text", "")).strip()
                        if text:
                            typer.echo(f"> {text}")
                    state["thinking_count"] = len(thinking)

                    current_content = message.content or ""
                    previous_content = str(state["content"])
                    if current_content != previous_content:
                        if not state["opened"]:
                            typer.echo(f"{message.author_name}: ", nl=False)
                            state["opened"] = True
                        if current_content.startswith(previous_content):
                            delta = current_content[len(previous_content) :]
                        else:
                            if state["opened"] and not state["closed"]:
                                typer.echo("")
                            typer.echo(f"{message.author_name}: ", nl=False)
                    ...
                    if metadata.get("streamState") in {"final", "error"} and state["opened"] and not state["closed"]:
                        typer.echo("")
                        state["closed"] = True
                    continue

                if message.id not in rendered_regular_ids:
                    typer.echo(f"{message.author_name}: {message.content}")
                    rendered_regular_ids.add(message.id)
        finally:
            db.close()
